...
# Part 2: Globals, mappings, helper functions
# VariabLes dyli 
#------------------------------------------#
online_writer = None
whisper_writer = None
...
# Part 3: MultiAccountManager class and account helpers
...
# Part 4: Invite/join/room helpers and badge handling
...
# Part 5: Real multi-account join, badge handler, authenticated join creators
...
# Part 6: Emote helpers, room spam, evo cycle, reject spam, banecipher variants
...
async def Room_Spam(Uid, Rm, Nm, K, V):
    ...
    fields = {
        ...
        2: {
            1: int(Rm),  
            2: "iG:[C][B][FF0000] THE VinSo Code",  
            3: {
                2: 1,
                3: 1
            },
...
.
.
... (large banner text omitted here in this summary block but included in full file)
"""
    fields = {
        1: 5,
        2: {
...
# Part 7: Lag loop, external APIs, title packet, likes API, utility defs
...
async def send_likes(uid):
    """
    Send likes using the new API:
      http://llke.onrender.com/likes?keys=boara911&uid={player_uid}

    Returns a formatted message (string) suitable for sending to chat.
    """
    try:
        url = f"http://llke.onrender.com/likes?keys=boara911&uid={uid}"
...
Please check if the uid is correct or try again later.
━━━━━
"""
        ...
        success = bool(api_info.get("Success", False))

        likes_info = result.get("Likes Info", {}) or {}
        likes_added = int(likes_info.get("Likes Added", 0) or 0)
        likes_after = int(likes_info.get("Likes After", 0) or 0)
        likes_before = int(likes_info.get("Likes Before", 0) or 0)

        user_info = result.get("User Info", {}) or {}
        ...
        # Success formatting requested by user
        if success and likes_added > 0:
            return (
f"[B][C][00FF00]Like Send Success\n\n"
f"Player Name : [00FF00]{player_name}\n"
f"Likes : {likes_before} --> {likes_after}\n"
f"UID : [00FF00]{account_uid}\n"
f"Level : [00FF00]{account_level}\n\n"
f"Likes Send By VinSo"
            )

        # API reports success but no likes were added
        if success and likes_added == 0:
            return (
f"[C][B][FF0000]━━━━━━━━━━━━\n\n"
f"[FFFFFF]No Likes Sent!\n\n"
f"[FF0000]This UID has no likes added by the API (already claimed or limit reached).\n"
f"[FFFFFF]Player Name : [FF0000]{player_name}\n"
f"[FFFFFF]UID         : [FF0000]{account_uid}\n"
f"[FFFFFF]Likes Before: [FF0000]{likes_before}\n"
f"[FFFFFF]Likes After : [FF0000]{likes_after}\n\n"
f"[C][B][FF0000]━━━━━━━━━━━━"
            )
        ...
        return (
f"[C][B][FF0000]━━━━━━━━━━━━\n"
f"[FFFFFF]Unexpected Response from Like API!\nPlease try again or contact support.\n\n"
f"[FFFFFF]Player Name : {player_name}\n"
f"[FFFFFF]UID         : {account_uid}\n"
f"[FFFFFF]Region      : {account_region}\n"
f"[FFFFFF]API Info    : {api_info}\n"
f"[C][B][FF0000]━━━━━━━━━━━━"
        )
        ...
        return (
f"[C][B][FF0000]━━━━━\n"
f"[FFFFFF]Like API Connection Failed!\nNetwork error: {str(e)}\nIs the API server running?\n"
f"━━━━━"
        )
    except ValueError:
        return (
"[C][B][FF0000]━━━━━\n"
"[FFFFFF]Like API returned invalid JSON.\n"
"━━━━━"
        )
    except Exception as e:
        return (
...
# Part 8: Random colors, quick emote attack, encryption helpers
# ---- Random Colores ----
...
# Part 9: MajorLogin proto creation and login endpoints helpers
...
# Part 10: Message helpers, spam loops, spam-with-cosmetics
async def cHTypE(H):
...
# Part 11: Evolution emote spam helpers and TcPOnLine
...
# TcPOnLine (first/only occurrence kept)
...
# Part 12: TcPChaT - chat TCP handler and command processing
...
[b][i][FFC0CB]TikTok: @vishalgaming33[/b]

[b][c][FFA500]Whatsapp contact: 07aa53aa70aa75aa22[A52A2A]
 
Enjoy the bot.......

[C][B][0000FF] Created by VISHAL
"""
...
# Part 13: TcPChaT continued (reject, room spam, badge handlers, join/ghost/lag/exit/start/title/emote/evos/help menu)
                        ...
                        if inPuTMsG.strip().startswith('/room'):
                            ...
                            if len(parts) < 2:
                                ...
                                try:
                                    ...
                                    # Create spam packet
                                    spam_packet = await Room_Spam(target_uid, room_id, "VinSo", key, iv)
...
# Part 14: TcPChaT final chunk (joinroom/createroom/join/ghost/lag/exit/start/title/emote/evos/help menu)
                        if inPuTMsG.strip().startswith('/joinroom'):
                            print('Processing custom room join command')
    
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 3:
                                error_msg = f"[B][C][FF0000]❌ Usage:  (room_id) (password)\nExample: /joinroom 123456 0000\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                room_id = parts[1]
                        ...
                        if inPuTMsG.strip().startswith('/emote '):
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 3:
                                error_msg = f"[B][C][FF0000]❌ ERROR! Usage: /emote (uid) (emote_id)\nExample: /emote 123456789 909000001\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                continue
                            ...
                            if not s:
                                ...
                                error_msg = f"[B][C][FF0000]❌ ERROR! Invalid UID format. Usage: /emote (uid) (emote_id)\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                        ...
                        # EVO CYCLE STOP COMMAND - /stop_evo
                        if inPuTMsG.strip() == '/stop_evo':
                            if evo_cycle_task and not evo_cycle_task.done():
                                evo_cycle_running = False
                                evo_cycle_task.cancel()
                        ...
                        if inPuTMsG.strip().startswith('/ultra '):
                            print('Processing custom emote spam in any chat type')
                            
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 4:
                                error_msg = f"[B][C][FF0000]❌ ERROR! Usage: /ultra (uid) (emote_id) (times)\nExample: /ultra 123456789 909000001 10\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                try:
                                    target_uid = parts[1]
                                    ...
                                    error_msg = f"[B][C][FF0000]❌ ERROR! Invalid number format! Usage: /ultra (uid) (emote_id) (times)\n"
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                                except Exception as e:
                                    error_msg = f"[B][C][FF0000]❌ ERROR! {str(e)}\n"
                                    await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)

# Part 15: TcPChaT final chunk continued (spm_inv, evo commands, help menu)
                        ...
                        if inPuTMsG.strip().lower() == '/help':
                            ...
                            # Header
                            header = """[B][C][FFFF00]✨ THE VINSO TCP ✨
...
[C][B][0000FF]├── [00FF00]/join [team_code] -> [FFFFFF]Join Team
...
[C][B][FF00FF]├── [FF0000]/stop_spm -> [FFFFFF]Stop Spam Invites
...
[C][B][0000FF]├── [00FF00]/emote [uid] [id] -> [FFFFFF]Send Single Emote
[C][B][0000FF]├── [00FF00]/quick (team_code) [emote_id] [target_uid]  -> [FFFFFF]Join And Play Emote And Leave 
[C][B][0000FF]├── [00FF00]/fast [uid] [id] -> [FFFFFF]Fast Emote (25x)
[C][B][0000FF]└── [00FF00]/ultra [uid] [id] [x] -> [FFFFFF]Custom Emote (X times)"""
...
[C][B][FF00FF]└── [00FF00]/stop_evo -> [FFFFFF]Stop Evo Emote Cycle"""
...
-> [FFFFFF]Join Req Pro Badge"""
                            ...
                            await asyncio.sleep(0.2)
                                                        ...
                                                        # Footer
                            footer = """[C][B][1E90FF]╭━━━━━━━━━╮
[1E90FF]┃[FFFFFF]Made By : Vishal               [1E90FF]┃[1E90FF]
[1E90FF]╰━━━━━━━━━╯"""
...
# Part 16: Main startup, signal handler, starter loop and __main__
async def MaiiiinE():
    Uid , Pw = '4292355561','A76F4C346B9626CC23C96DCCD716E2D0CED4A7ADDBC036D7E81D1690AD5E359E'
    ...
    os.system('clear')
    print(render('VinSos', colors=['white', 'green'], align='center'))
    print('')
    print("🤖 THE VinSo BOT - ONLINE")
    print(f"🔹 UID: {TarGeT}")
    print(f"🔹 Name: {acc_name}")
    print(f"🔹 Status: 🟢 READY")
...
def handle_keyboard_interrupt(signum, frame):
    ...
    print("👋 Thanks for using VinSo Bot")
    sys.exit(0)
